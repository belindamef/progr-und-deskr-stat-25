...
\textbf{19.11.24}       & \textbf{R Grundlagen}             & \textbf{(6) Datenmanagement}                    & \textbf{Seminar}           \\
...
data_dir_path <- file.path(work_dir_path, "Daten")           # Datenverzeichnispfad
...
\begin{enumerate}
\justifying
\item Dokumentiere alle in dieser Einheit verwendeten R-Befehle in einem R-Skript. 
\item  Lass dir dein aktuelles Working Directory ausgeben. Ändere es anschließend zu einem anderen Ordner, überprüfe mit einer erneuten Ausgabe, ob die Änderung erfolgreich war. Setze das Working Directory danach wieder zurück und überprüfe erneut, ob die Rücksetzung funktioniert hat.
\item Schreibe ein einfaches Skript, in dem Daten in ein Dataframe eingelesen werden. Füge neue Daten hinzu und speichere das resultierende Dataframe in einer neuen Datei. Verwende dabei Pfadvariablen (d.h., einen ein-elementigen character vector für den Dateipfad).
\item Wiederhole die vorherige Aufgae, jedoch mit einer geänderten Pfadvariable.
  \begin{itemize} \small
  \item Ändere dafür zunächst dein Working Directory in ein anderes Verzeichnis, z.B. in das übergeordnete Verzeichnis (parent directory). 
  \item Passe anschließend die Pfadvariable im Code an, die du zum Einlesen der Daten verwendest, damit sie zum neuen Working Directory passt. 
  \item Beachte, dass sich in deiner Ordnerstruktur auf der Festplatte nichts ändert, sondern lediglich das \textquote{Working Directory} und die Pfadvariable angepasst werden.
  \end{itemize}
\end{enumerate}
