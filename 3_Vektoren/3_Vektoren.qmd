...
\textbf{05.11.24}       & \textbf{R Grundlagen}             & \textbf{(3) Vektoren}, \textbf{(4) Matrizen}             & \textbf{Seminar}           \\
...
\tiny
```{r, eval = F}
#| label: Mehrlementige Vektoren Beispiele
...
Out-of-range Indizes verursachen keine Fehler, sondern geben \textcolor{blue}{NA} aus
```{r, eval = F}
...
* \footnotesize Arithmetik von Vektoren und Skalaren ist ein Spezialfall dieses Prinzips.
  ```{r, eval = F}
...
# Fehlende Werte (\textcolor{blue}{NA})
\small
\setstretch{1.4}

* \textcolor{blue}{NA} steht für “**N**ot **A**vailable” und repräsentiert fehlende Werte in R

* Das Rechnen mit \textcolor{blue}{NA}s ergibt (meist) wieder \textcolor{blue}{NA}. \setstretch{0.9} \footnotesize
  ```{r, eval = T}
  #| label: multiply NA
...
* Auf \textcolor{blue}{NA} testet man mit `is.na()` oder \texttt{anyNA()}\footnotesize 
  ```{r, eval = T}
...
# Ungültige Zahlenwerte (\textcolor{blue}{NaN})
\small
\setstretch{1.4}

* \textcolor{blue}{NaN} steht für “**N**ot **a** “**N**umber” repräsentiert ungültige Zahlenwerte, z.B. durch undefinierte arithmetische Operationen \footnotesize 
  ```{r, eval = T}
...
* \small In Mathematik und Informatik ist Dividieren durch Null undefiniert. R verwendet jedoch das Konzept "Unendlichkeit": positive Zahlen gehen bei Division durch Null gegen positive Unendlichkeit \textcolor{blue}{(Inf)}, negative gegen negative \textcolor{blue}{(-Inf)}, ähnlich wie Grenzwerte in der Analysis. \footnotesize \setstretch{1.2}
  ```{r, eval = T}
...
* Das Rechnen mit \textcolor{blue}{NaN} ergibt (meist) wieder \textcolor{blue}{NaN}. \setstretch{0.9} \footnotesize
  ```{r, eval = T}
...
* Auf \textcolor{blue}{NaN} testet man mit `is.nan()` \footnotesize 
  ```{r, eval = T}
...
# Leere Werte (\textcolor{blue}{NULL})
\small
\setstretch{1.4}

* \textcolor{blue}{NULL} zeigt die völlige Abwesenheit eines Werts an, oft genutzt für leere Objekte. \footnotesize
  ```{r, eval = T}
  w <- c()                 # Leerer Vektor
  print(w)
  ```
...
* Das Rechnen mit \textcolor{blue}{NULL} ergibt leere Objekte \setstretch{0.9} \footnotesize
  ```{r, eval = T}
...
* Auf \textcolor{blue}{NULL} testet man mit `is.null()` \footnotesize 
  ```{r, eval = T}
...
# \textcolor{blue}{NA}, \textcolor{blue}{NaN} und \textcolor{blue}{NULL}
\small
\setstretch{1.4}

* \textcolor{blue}{NA} repräsentiert allgemein **fehlende** Werte.
* \textcolor{blue}{NaN} repräsentiert spezifisch für **ungültige** numerische Zahlenwerte, etwa das Ergebnis undefininierter arithmetischer Operationen.
* \textcolor{blue}{NULL} representiert ein leeres Objekt.
* Alle drei Begriffe \textcolor{blue}{NA}, \textcolor{blue}{NaN} und \textcolor{blue}{NULL} zählen zu den *reserverd words* (siehe `?reserved`).
* \textcolor{blue}{NaN} wird auch von \texttt{is.na()} als fehlender Wert erfasst. Umgekehrt zählt \texttt{is.nan()} jedoch nur numerisch ungültige Werte (\textcolor{blue}{NaN}) und schließt allgemeine fehlende Werte (\textcolor{blue}{NA}) aus. \footnotesize \setstretch{1.0}
  ```{r, eval = T}
  print(y)
  ```
  \vspace{1mm}
...
\tiny
```{r, eval = T}
print(                   # Äußere Funktion, auf Ebene 0
  c(                     # Innere Funktion (=1. Argument der äußeren Funktion), auf Ebene 1
    sieben = 7.421,      # 1. Argument der inneren Funktionen auf Ebene 2; Komma da 2. Arg folgt!
    neun   = 9.234       # 2. Argument der inneren Funktionen auf Ebene 2
  ),                     # Schließen der inneren Funktion auf Ebene 1; Komma, da 2. Arg folgt!
  digits=1               # 2. Argument der äußeren Funktion, auf Ebene 1
)                        # Schließen der äußerden Funktion auf Ebene 0
```
...
\begin{enumerate} \justifying
  ...
  \item Öffne die R Help Seite der Funktion \texttt{c()}. Zu welchem R-Paket gehört \texttt{c()}?
  \item Erstelle einen Vektor mit den Zahlen 1, 2, 3 mit dem Colonoperator.
  \item Überlege dir zum jedem der vier Prinzipien der Indizierung in R ein Beispiel und dokumentiere es im R Skript aus Aufg 1.
  ...
  \item Erstelle einen Vektor, der sowohl Zahlenwerte als auch fehlende Werte (\texttt{NA}) enthält, zum Beispiel \texttt{(1, 2, NA, 19, NA, 41)} und berechne die Summe aller Werte dieses Vektors mit der Funktion \texttt{sum()}. \footnotesize
  \begin{itemize}\footnotesize
    \item Warum gibt die Funktion als Ergebnis \textcolor{blue}{NA}? Recherchiere dazu auf der Help Page (\texttt{"?sum"}).
    \item Wie kannst du die Funktion \texttt{sum()} dazu veranlassen, die \textcolor{blue}{NA}-Werte zu ignorieren und die Summe nur über die vorhandenen Zahlenwerte zu berechnen?
  \end{itemize}
\end{enumerate}
