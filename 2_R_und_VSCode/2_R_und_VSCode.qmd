...
\textbf{29.10.24}       & \textbf{R Grundlagen}             & \textbf{(2) R und Visual Studio Code}           & \textbf{Übung}             \\
...
\begin{itemize}
\item Beginne ein R Skript mit einem \textbf{Header}, welche die wichtigsten Informationen zu dem vorliegenden Programm enthält.
\item Achte darauf, möglichst jeden Befehl mit einem \textbf{Kommentar} zu versehen, sodass leicht nachvollzogen werden kann, was in jeder Zeile passiert.
\end{itemize}
...
\begin{itemize} \small
  ...
  \item Tipp: Integriere das Paket-Management in dein R Skript. 
  \begin{itemize} \small
    ...
    \item Lade alle benötigten Pakete zu Beginn des Skripts und halte in einem Kommentar fest, wofür es benötigt wird.
  \end{itemize}
\end{itemize}
...
\begin{enumerate}
\item Erkunde die VSCode User Interface. (Tipp: Konsultiere die offizielle \textcolor{linkblue}{\href{https://code.visualstudio.com/docs/getstarted/userinterface}{VSCode Online-Doku}})
\begin{enumerate} \footnotesize
\item Welche verschiedenen Views können in der \textit{Activity Bar} ausgewählt werden?
\item Wähle in der \textit{Activity Bar} den \textit{Explorer} aus. Wie kannst du einstellen, dass der Kursordner, den du letzte Woche erstellt hast, in der \textit{Primary Sidebar} sichtbar ist?
\item Wähle in der \textit{Activity Bar} die \textit{R Extension} aus. Was ist nun alles in der \textit{Primary Sidebar} zu sehen?
\item Wie kannst du konfigurieren, dass das \textit{Panel} inklusive R-Console links und nicht unten lokalisiert ist?
\item Wie kannst du eine \textit{Secondary Sidebar} öffnen?
\end{enumerate}
\item Gib ein paar einfache Befehle in die Console ein.
\item Öffne die Help-Page für die Funktion \texttt{print()}. Was ist der Unterschied zwischen den Befehlen \texttt{'print(x="Hallo Welt")'} und \texttt{'print("Hallo Welt")'}?
\item Was kann über das Argument \texttt{digits} spezifiziert werden? Suche unter \texttt{\textbf{Examples}} ein Beispiel für die Verwendung dieses Arguments. Was ist der Unterschied zwischen den Befehlen \texttt{'print(pi)'} und \texttt{'print(pi, digits=2)'}?
\item Gib unvollständige Befehl in die Console ein und drücke die \fbox{Enter}-Taste. \\ (Beispiele: \texttt{'print("Hello"'} oder \texttt{'sqrt(4'}, \texttt{'x <-'} oder \texttt{'sqrt(1 +'}). Was passiert in der Konsole? Wie kannst du den prompt '>' wiederherstellen?
...
\item Gib den Befehl \texttt{'sum("Hello World!")'} ein und erläutere die resultierende Fehlermeldung.
\end{enumerate}
...
\item Was passiert in der Console, wenn du auf das Kreuz neben der Variable klickst?
